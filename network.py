...
class VDSR:
    def __init__(self, sess, args):
        ...
        if args.phase == 'train':
            ...
            self.train_HR, self.train_LR = load_291(scale=self.train_scale)
    ...
    def network(self, x, reuse=False):
        with tf.variable_scope("VDSR", reuse=reuse):
            for d in range(0,self.network_depth-1):
                x = conv(x, 64, kernel=3, stride=1, padding='SAME', use_bias=True, scope='conv_' + str(d))
                x = relu(x)

            x = conv(x, 1, kernel=3, stride=1, padding='SAME', use_bias=True, scope='conv_' + str(self.network_depth-1))
    ...
    def build_model(self):
        ...
        self.train_output = tf.clip_by_value(self.train_logits + self.train_X, self.pixel_min, self.pixel_max)
        self.test_output = tf.clip_by_value(self.test_logits + self.test_X, self.pixel_min, self.pixel_max)

        self.train_loss = tf.reduce_mean(tf.square((self.train_Y - self.train_X - self.train_logits)))
        self.test_loss  = tf.reduce_mean(tf.square((self.test_Y - self.test_X - self.test_logits)))
        ...
        capped_gvs = [(tf.clip_by_norm(grad, self.grad_clip/self.lr), var) for grad, var in gvs]
    ...
    def train(self):
        ...
        if(start_epoch == self.epoch // 4):
            epoch_lr /= 10
        elif(start_epoch == self.epoch // 4 * 2):
            epoch_lr /= 100
        elif(start_epoch == self.epoch // 4 * 3):
            epoch_lr /= 1000

        start_time = time.time()
        for epoch in range(start_epoch, self.epoch):
            
            if epoch == self.epoch//4 or epoch == self.epoch//4*2 or epoch == self.epoch//4*3:
    ...
    @property
    def model_dir(self):
        return "{}_{}_{}_{}_{}".format(self.model_name, scale2str(self.train_scale), self.channel, self.batch_size, self.grad_clip)
